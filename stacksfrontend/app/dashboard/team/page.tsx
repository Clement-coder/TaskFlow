  ...
  return (
    <div className="space-y-7 pb-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">Team</h1>
          <p className="text-sm text-slate-400 mt-1">Manage members, roles, and permissions</p>
        </div>
        <button
          onClick={() => setShowForm(!showForm)}
          className="flex items-center justify-center gap-2 rounded-xl bg-sky-500 hover:bg-sky-400 active:bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-sky-500/25 transition duration-150"
        >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-xl bg-slate-900/60 border border-white/[0.07] pl-10 pr-4 py-2.5 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-sky-500/60 focus:ring-2 focus:ring-sky-500/20 transition"
        />
      </div>

      {/* Members table */}
      <div className="rounded-2xl border border-white/[0.07] bg-slate-900/60 overflow-hidden shadow-md">
        <div className="hidden sm:grid grid-cols-[1fr_auto_auto_auto_auto] gap-4 px-5 py-3 border-b border-white/[0.06] text-[10px] uppercase tracking-[0.12em] font-bold text-slate-500">
          <span>Member</span>
        ...
        <div className="divide-y divide-white/[0.04]">
          {filtered.map((member) => (
            <div key={member.id} className="flex flex-col sm:grid sm:grid-cols-[1fr_auto_auto_auto_auto] gap-3 sm:gap-4 px-5 py-4 items-start sm:items-center hover:bg-white/[0.02] transition">
              ...
              <p className="text-sm font-semibold tabular-nums text-sky-400 sm:text-center">{member.reputation}</p>
              <div className="sm:text-center">
                {member.role !== "Owner" && (
                  <button
